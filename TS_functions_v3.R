# THESE FUNCTIONS PROCESS HABITUATION, SHAPING AND EXPERIMENTAL DATA
...
format2dataframe = function(data = ml) {
    ...
    for(ii in 1:nrow(details)) {
        ...
        data_t = dplyr::tibble(trialOrder = numeric(trialLength), trialType = character(trialLength), 
                               ...
                               outcome = character(trialLength), postChoiceVisit = logical(trialLength),
...
extract_latency = function(data = ml, exclude = NULL) {
    ...
    latency_df = dplyr::tibble(fileName = character(n),
                               correctResponseLatency = numeric(n), 
                               incorrectResponseLatency = numeric(n), 
                               generalResponseLatency = numeric(n))
    ...
    for(ii in 1:n) {
        ...
        latency_df[ii, "correctResponseLatency"] = response_c_ic[1]
        latency_df[ii, "incorrectResponseLatency"] = response_c_ic[2]
        latency_df[ii, "generalResponseLatency"] = mean(response_all$Response_Latency)
    }
...
extract_rewardCollectionError = function(data = ml, exclude = NULL) {
    ...
    prop2tray = dplyr::tibble(fileName = character(n),
                              propTrayCorrect = numeric(n), propTrayIncorrect = numeric(n))
    ...
    for(ii in 1:n) {
        ...
        prop2tray[ii, "propTrayCorrect"] = prop_c_ic[1]
        prop2tray[ii, "propTrayIncorrect"] = prop_c_ic[2]
    }
...
extract_screenTouches = function(data = ml, exclude = NULL) {
    ...
    screenActivity_df =
        df %>% 
        select(postScreenTouch, everything()) %>% 
...
process_sequence = function(seq = seq, details = ml, test = NULL, start = NULL, end = NULL, title = NULL, max_length = 40) {
    ...
    seqPlot =
        ...
        annotate("text", label = paste("KS test: p =", ks.test(seq_list[[1]],
                                                               seq_list[[2]])$p.value %>% round(2)), y = 0, x = max_length, hjust = 1) +
